...
export default function Chat() {
 ...
 // Effect to handle dynamic email reception
 // Effect to handle dynamic email reception
 useEffect(() => {
  console.log("Setting up message listener for email...");

    const handleMessage = (event) => {
        console.log("Message received in iframe:", event.data); // Log the entire event data
        
        // Directly log the message without any checks
        if (event.data && event.data.email) {
            console.log(`Received email: ${event.data.email}`);
            setUserEmail(event.data.email); // Update the state with the received email
        } else {
            console.log("Message received but no email found:", event.data);
        }
    };
...
// Effect to initiate API calls once the email is set
useEffect(() => {
  if (userEmail) {
    console.log(`Email is set, proceeding with API calls: ${userEmail}`);
    initiateApiCalls(userEmail); // Trigger the API calls
  } else {
    console.log('Still waiting for the email to be set...');
  }
}, [userEmail]);

const initiateApiCalls = async (email) => {
  console.log(`Initiating API calls with email: ${email}`);
  await fetchEmailAndChatHistory(email); // Fetch the chat history after confirming the email is set
};



const fetchEmailAndChatHistory = async (email) => {
  try {
    ...
    const res = await fetch('/api/handle-email', {
      ...
      body: JSON.stringify({ email: email }), // Pass the email dynamically
    });

    const data = await res.json();

    if (data.success && data.email) {
      console.log(`Server responded with email: ${data.email}`);
      fetchChatHistory(data.email); // Proceed to fetch chat history with the dynamic email
    } else {
      console.error('Failed to receive email:', data.message);
    }
  } catch (error) {
...
const fetchChatHistory = async (email) => {
  try {
    ...
    if (data.success) {
      console.log('Chat history fetched successfully:', data.history.sessions);
      setSessions(data.history.sessions || []);
    } else {
      console.error('Failed to fetch sessions:', data.message);
    }
  ...
  const handleSendMessage = async () => {
    ...
    const formData = new FormData();
    formData.append('email', userEmail); // Include email
